...
public class JwtUtil {
    ...
    /**
     * 创建 Jwt 签名算法：HS256 - HMAC with SHA-256, requires 256+ bit secret
     *
     * @param claims
     *            负载
     * @param secret
     ...
     * @return Jwt
     */
    public static String createJwtWithHS256(Map<String, Object> claims, String secret, long liveTime,
            ChronoUnit timeUnit) {
        return Try.of(() -> {
            ...
            claimSetBuilder.expirationTime(
                    Date.from(signTime.plus(liveTime, timeUnit).atZone(ZoneId.systemDefault()).toInstant()));
    ...
    /**
     * 校验 Jwt 签名算法：HS256 - HMAC with SHA-256, requires 256+ bit secret
     *
     * @param jwt
     *            jwt
     * @param secret
    ...
    public static boolean verifyJwtWithHS256(String jwt, String secret) {
        return Try.of(() -> {
            ...
            return signedJWT.verify(jwsVerifier) && LocalDateTime.now().isBefore(signedJWT.getJWTClaimsSet()
