 ...
 *
 */
public class HighPasswordStrengthChecker implements PasswordStrengthChecker {

    @Override
    public Boolean validate(String password) {
        PasswordRuleManager ruleManager = new PasswordRuleManager();
        ruleManager.addRule(new LengthPasswordRule(6, -1));
        ruleManager.addRule(new CharTypePasswordRule(List.of(CharType.LETTER, CharType.DIGIT, CharType.SPECIAL_CHAR), 3));
        return ruleManager.validateAll(password);
    }

    @Override
    public String errorMessage() {
        return "请使用至少 6 位字符作为密码，且密码中须包含英文、数字与符号。";
    }
}
