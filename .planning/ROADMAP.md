...
**Success Criteria** (what must be TRUE):
  ...
  2. 管理员可以在详情页批准/拒绝申请，可选择限制条件
  3. 管理员可以拒绝申请，必须填写拒绝理由
  4. 管理员可以部分批准/部分拒绝同一申请单内的子项
  5. 管理员不能审批自己的申请 (后端拦截)
**Plans**: 3 plans

Plans:
- [x] 10-01-PLAN.md — 实现管理员待审批列表和全部申请页面
- [x] 10-02-PLAN.md — 实现申请详情页面（路由 /permission/request/detail?id=），含部分批准/拒绝功能
- [x] 10-03-PLAN.md — 实现部分批准+部分拒绝工作流，支持批准其余/拒绝其余按钮
