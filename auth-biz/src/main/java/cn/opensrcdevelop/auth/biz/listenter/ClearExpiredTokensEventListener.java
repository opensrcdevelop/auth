...
public class ClearExpiredTokensEventListener implements ApplicationListener<ClearExpiredTokensEvent> {
     ...
     */
    private void clearRefreshToken(List<OAuth2Authorization> oAuth2AuthorizationList,
            List<OAuth2Authorization> deleteTargetList) {
     ...
     */
    private void clearAccessTokenWithoutRefreshToken(List<OAuth2Authorization> oAuth2AuthorizationList,
            List<OAuth2Authorization> deleteTargetList) {
        oAuth2AuthorizationList.forEach(authorization -> {
            if (Objects.nonNull(authorization.getAccessToken()) && Objects.isNull(authorization.getRefreshToken())
     ...
     */
    private void clearAccessTokenWithRefreshToken(List<OAuth2Authorization> oAuth2AuthorizationList,
            List<OAuth2Authorization> deleteTargetList) {
        ...
        if (MapUtils.isNotEmpty(groupRes)) {
            ...
            groupRes.keySet()
                    .forEach(key -> groupRes.computeIfPresent(key,
                            (k, v) -> v.stream().sorted(Comparator.comparing(OAuth2Authorization::getId))
            ...
            CommonUtil.stream(groupRes.values())
                    .forEach(authorizationList -> authorizationList.forEach(authorization -> {
     ...
     */
    private void clearAuthorizationCode(List<OAuth2Authorization> oAuth2AuthorizationList,
            List<OAuth2Authorization> deleteTargetList) {
        oAuth2AuthorizationList.forEach(authorization -> {
