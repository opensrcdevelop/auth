...
public class IdentitySourceRegistrationServiceImpl
        extends
            ServiceImpl<IdentitySourceRegistrationMapper, IdentitySourceRegistration>
        implements
            IdentitySourceRegistrationService {
     ...
     *            请求
     */
    @Audit(type = AuditType.SYS_OPERATION, resource = ResourceType.IDENTITY_SOURCE, sysOperation = SysOperationType.CREATE, success = "创建了身份源（{{ #registrationId }}）", fail = "创建身份源（{{ #requestDto.name }}）失败，对应的提供商为：{{ @linkGen.toLink(#requestDto.providerId, T(ResourceType).IDENTITY_SOURCE_PROVIDER) }}")
    @Transactional
    @Override
     ...
     *            请求
     */
    @Audit(type = AuditType.SYS_OPERATION, resource = ResourceType.IDENTITY_SOURCE, sysOperation = SysOperationType.UPDATE, success = "修改了身份源（{{ @linkGen.toLink(#requestDto.id, T(ResourceType).IDENTITY_SOURCE) }}）", fail = "修改身份源（{{ @linkGen.toLink(#requestDto.id, T(ResourceType).IDENTITY_SOURCE) }}）失败")
    @Transactional
    @Override
     ...
     *            身份源 ID
     */
    @Audit(type = AuditType.SYS_OPERATION, resource = ResourceType.IDENTITY_SOURCE, sysOperation = SysOperationType.DELETE, success = "删除了身份源（{{ @linkGen.toLink(#id, T(ResourceType).IDENTITY_SOURCE) }}）", fail = "删除身份源（{{ @linkGen.toLink(#id, T(ResourceType).IDENTITY_SOURCE) }}）失败")
    @Override
    ...
    public List<IdentitySourceRegistrationResponseDto> getBoundRegistrations() {
        ...
        // 3. 获取用户已绑定的身份源ID
        List<ThirdAccount> thirdAccounts = thirdAccountService
                .list(Wrappers.<ThirdAccount>lambdaQuery().eq(ThirdAccount::getUserId, userId));
     ...
     */
    @Override
    public UserBindingResponseDto bindUser(String registrationCode, HttpServletRequest request,
            HttpServletResponse response) throws IOException {
        UserBindingResponseDto responseDto = UserBindingResponseDto.builder().build();
        // 1. 检查身份源是否存在
        IdentitySourceRegistration registration = super.getOne(Wrappers.<IdentitySourceRegistration>lambdaQuery()
                .eq(IdentitySourceRegistration::getRegistrationCode, registrationCode));
        if (Objects.isNull(registration)) {
     ...
     *            身份源ID
     */
    @Audit(type = AuditType.USER_OPERATION, resource = ResourceType.IDENTITY_SOURCE, userOperation = UserOperationType.UNBIND_THIRD_ACCOUNT, success = "解绑了身份源（{{ @linkGen.toLink(#registrationId, T(ResourceType).IDENTITY_SOURCE) }}）", fail = "解绑身份源（{{ @linkGen.toLink(#registrationId, T(ResourceType).IDENTITY_SOURCE) }}）失败")
    @Override
    public void unbindUser(String registrationId) {
        ...
        // 2. 删除第三方账号
        thirdAccountService.remove(Wrappers.<ThirdAccount>lambdaQuery()
                .eq(ThirdAccount::getRegistrationId, registrationId).eq(ThirdAccount::getUserId, userId));
    }

    private void checkRegistrationCode(IdentitySourceRegistrationRequestDto requestDto,
            IdentitySourceRegistration rawRegistration) {
        if (Objects.nonNull(rawRegistration)
                && StringUtils.equals(requestDto.getCode(), rawRegistration.getRegistrationCode())) {
            return;
        }

        if (Objects.nonNull(super.getOne(Wrappers.<IdentitySourceRegistration>lambdaQuery()
                .eq(IdentitySourceRegistration::getRegistrationCode, requestDto.getCode())))) {
