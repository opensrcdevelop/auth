import { getTaskList, getTask, downloadTaskResult, type AsyncTask, type TaskType, type TaskStatus } from "@/api/asyncTask";
import { Message } from "@arco-design/web-vue";
import { defineComponent, onMounted, onUnmounted, ref } from "vue";
import { usePagination } from "@/hooks/usePagination";
import { useTaskNotification, type TaskNotificationMessage } from "@/hooks/useTaskNotification";
 ...
 * 任务类型选项 - 由数据驱动
 */
export const taskTypeOptions: { label: string; value: TaskType }[] = [
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleTaskNotification = (message: TaskNotificationMessage) => {
      // 更新任务列表中的任务状态
      const index = taskList.value.findIndex((task) => task.taskId === message.taskId);
      if (index !== -1) {
    ...
    /** 分页 hook */
    const taskListPagination = usePagination("taskList", fetchTaskList);
